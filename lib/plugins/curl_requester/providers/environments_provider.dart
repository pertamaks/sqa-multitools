...
class Environments extends _$Environments {
  ...
  List<Environment> build() {
    try {
      ...
      final envsJson = prefs.rawPrefs.getString(PreferencesService.keyCurlEnvironments);
      
      if (envsJson != null) {
        final decoded = jsonDecode(envsJson) as List;
        return decoded.map((e) => Environment.fromJson(e as Map<String, dynamic>)).toList();
      }
    } catch (_) {}
    
    // Default empty environment
    return [
      const Environment(id: 'default', name: 'Global', variables: {})
    ];
  }
...
class ActiveEnvironmentId extends _$ActiveEnvironmentId {
  ...
  String build() {
    final prefs = ref.read(preferencesServiceProvider);
    return prefs.rawPrefs.getString('${PreferencesService.keyCurlEnvironments}_active_id') ?? 'default';
  }

  void setActiveId(String id) {
    state = id;
    ref.read(preferencesServiceProvider).rawPrefs.setString('${PreferencesService.keyCurlEnvironments}_active_id', id);
  }
}
