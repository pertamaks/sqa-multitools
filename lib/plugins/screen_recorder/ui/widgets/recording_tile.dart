...
class _RecordingTileState extends State<RecordingTile> {
  ...
  Widget build(BuildContext context) {
    ...
    // We use NotificationListener to detect when the popup menu is opened or closed.
    // This allows us to clear the hover state forcefully, solving the Flutter overlay bug.
    return NotificationListener<SqaMenuOpenNotification>(
      onNotification: (notification) {
        setState(() {
          _isMenuOpen = notification.isOpen;
          if (_isMenuOpen) {
            _isHovered = false; // Force clear hover when menu opens
          }
        });
        return false; // Let it bubble up just in case
      },
      child: MouseRegion(
        onEnter: (_) {
