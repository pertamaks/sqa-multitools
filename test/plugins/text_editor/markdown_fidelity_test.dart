...
void main() {
  group('SqaQuoteNodeParser', () {
    test(
      'should transform quote node to markdown with > prefix and double newline',
      () {
        // 1. Arrange: Create a quote node with a delta
        final node = Node(
          type: QuoteBlockKeys.type,
          attributes: {'delta': (Delta()..insert('Hello SQA World')).toJson()},
        );
    ...
    test('should return empty string if delta is missing', () {
      final node = Node(type: QuoteBlockKeys.type, attributes: {});
  ...
  group('SqaDeltaMarkdownEncoder', () {
    ...
    test('should encode colors using HTML spans', () {
      ...
      final encoder = SqaDeltaMarkdownEncoder();
      expect(
        encoder.convert(delta),
        equals(
          '<span style="color:#ff0000;background-color:#ffff00;">colored</span>',
        ),
      );
    });
    ...
    test('should encode combined styles (bold + color)', () {
      ...
      expect(
        encoder.convert(delta),
        equals('**<span style="color:#ff0000;">both</span>**'),
      );
    });
  });
}
