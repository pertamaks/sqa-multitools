...
class SqaMarkdownTableParser extends CustomMarkdownParser {
  ...
  List<Node> transform(
    ...
    final headElement = ec.whereType<md.Element>().where((e) => e.tag == 'thead').firstOrNull;
    if (headElement != null) {
      final rows = headElement.children?.whereType<md.Element>().where((e) => e.tag == 'tr') ?? [];
      for (final tr in rows) {
        allRows.add(tr.children?.whereType<md.Element>().where((e) => (e.tag == 'th' || e.tag == 'td')).toList() ?? []);
      }
    }

    final bodyElement = ec.whereType<md.Element>().where((e) => e.tag == 'tbody').firstOrNull;
    if (bodyElement != null) {
      final rows = bodyElement.children?.whereType<md.Element>().where((e) => e.tag == 'tr') ?? [];
      for (final tr in rows) {
        allRows.add(tr.children?.whereType<md.Element>().where((e) => e.tag == 'td').toList() ?? []);
      }
    }
    ...
    for (int col = 0; col < colsLen; col++) {
      for (int row = 0; row < rowsLen; row++) {
        ...
        if (row == 0) {
          ...
          int alignment = 0;
          if (alignAttr == 'center' || styleAttr.contains('text-align: center')) {
            alignment = 1;
          } else if (alignAttr == 'right' || styleAttr.contains('text-align: right')) {
            alignment = 2;
          }
    ...
    if (alignments.isNotEmpty) {
      tableNode.node.updateAttributes({
